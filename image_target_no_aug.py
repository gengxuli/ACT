...
def data_load(args): 
    ...
    if args.few_shot:
        ...
        dsets["target"] = utils._SplitDataset(train_dataset, tr_idx)
        dsets["test"] = utils._SplitDataset(test_dataset, te_idx)
        dsets["target"].transform = image_train()
        dsets["test"].transform = image_test()
        keys = list(range(len(dsets["target"])))
        targets = torch.tensor(dsets["target"].underlying_dataset.targets)[dsets["target"].keys]
...
def train_target(args):
    ...
    for epoch_num in range(max_epoch + 1):
        ...
        for step, (inputs_test, tar_idx) in enumerate(dset_loaders["target"]):
            netF.train()
            netC1.train()
            netC2.train()
            #netD.train()
