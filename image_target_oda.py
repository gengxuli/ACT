...
def data_load(args): 
    ...
    if not args.da == 'uda':
        ...
        tar_list = known_tar.copy()
        test_list = all_tar.copy()

    if args.few_shot:
        train_dataset = ImageList_idx(tar_list)
        test_dataset = ImageList_idx(test_list)
        ...
        targets = torch.tensor([train_dataset.imgs[i][1] for i in tr_idx])
        ...
        te_idx = list(set(te_idx).difference(set(few_shot_idx)))
...
def cal_acc_oda(loader, netF, netC1, netC2, flag=False):
    start_test = True
    with torch.no_grad():
...
def train_target(args):
    ...
    if args.SAM:
        if args.optim == 'adam':
            if args.dset == 'office-home':
                ...
                optimizer_c = SAM(param_group_c, torch.optim.Adam, lr=args.lr * 10 / 3, weight_decay=args.wd, rho=args.rho)
            else:
                optimizer_f = SAM(param_group_f, torch.optim.Adam, lr=args.lr, weight_decay=args.wd, rho=args.rho)
                optimizer_c = SAM(param_group_c, torch.optim.Adam, lr=args.lr * 10 / 3, weight_decay=args.wd, rho=args.rho)
        else:
    ...
    for epoch_num in range(max_epoch + 1):
        ...
        for step, (inputs_w, inputs_s, inputs_s1, tar_idx) in enumerate(dset_loaders["target"]):
            ...
            #adjust_learning_rate(optimizer_f, epoch_num // args.interval, step, len(dset_loaders["target"]), 0.003) 
            ...
            inputs_w = inputs_w[tar_idx<args.class_num, :]
            inputs_s = inputs_s[tar_idx<args.class_num, :]
            inputs_s1 = inputs_s1[tar_idx<args.class_num, :]
            tar_idx = tar_idx[tar_idx<args.class_num]
        ...
        if epoch_num % interval_iter == 0 or epoch_num == max_epoch:
            ...
            if args.dset=='VISDA-C':
                acc_os1, acc_os2, acc_unknown, acc_list = cal_acc_oda(dset_loaders['test'], netF, netC1, netC2, True)
                log_str = '\nTask: {}, Iter:{}/{}; Accuracy = {:.2f}% / {:.2f}% / {:.2f}%'.format(args.name, epoch_num, max_epoch, acc_os2, acc_os1, acc_unknown) + '\n' + acc_list
            else:
                acc_os1, acc_os2, acc_unknown = cal_acc_oda(dset_loaders['test'], netF, netC1, netC2, True)
                log_str = '\nTask: {}, Iter:{}/{}; Accuracy = {:.2f}% / {:.2f}% / {:.2f}%'.format(args.name, epoch_num, max_epoch, acc_os2, acc_os1, acc_unknown)
